...
Now let's introduce Twitter into the mix by using its API to grab a random trending hashtag and insert it into our poem. You will need the consumer key and consumer secret, access token, and access secret from the Twitter application you set up at <https://apps.twitter.com>. For the code below to work, in the settings of your application (click the "settings tab" at the URL above), the `Callback URL` should be set to `http://127.0.0.1:1410`.  
...
The code below words almost identically to our first mad-lib poem, but the poem is a bit longer (now that Twitter allows 280 characters) and instead of inserting a random interjection from Wordnik at the end, it instead samples one of the trending topics pulled from Twitter above and inserts that as the final word in the poem.
...
Then, so long as the resulting poem is less than 280 characters, we can post it to Twitter. The code below will check if the string `poem` is less than 280 characters and post if it is. If not, it will print a message asking you to rerun the poem generator. We could write this in a slightly more complicated way so the script would automatically rerun the poem generator until it created a poem short enough to tweet.
